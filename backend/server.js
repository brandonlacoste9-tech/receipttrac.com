...
const authenticateToken = (req, res, next) => {
  ...
  if (!token)
    return res
      .status(401)
      .json({ error: "Vault access denied. No token provided." });

  jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
    if (err)
      return res
        .status(403)
        .json({ error: "Vault security breach. Invalid token." });
    req.user = user;
    next();
  });
};
...
const requireSecureSession = (req, res, next) => {
  if (!req.user || !req.user.is_secure) {
    return res.status(403).json({
      error: "Sovereign clearance required. Please verify biometrics.",
    });
  }
  next();
};
...
app.post(
  ...
  async (req, res) => {
    console.log(
      `[VAULT] Generating registration options for User: ${req.user.email}`,
    );
    try {
      ...
      console.log(
        `[VAULT] Challenge generated and stored in session: ${options.challenge}`,
      );
      res.json(options);
    } catch (error) {
...
app.post(
  ...
  async (req, res) => {
    ...
    console.log(
      `[VAULT] Verifying registration. Expected Challenge: ${expectedChallenge}`,
    );
    ...
    try {
      ...
      if (verification.verified) {
        ...
        console.log(
          `[VAULT] Biometric signature successfully anchored for ${req.user.email}`,
        );
        res.json({ verified: true });
      } else {
        console.error(
          "[VAULT] Verification failed: verified property is false",
        );
...
app.post("/api/auth/biometric/login-options", async (req, res) => {
  ...
  try {
    ...
    if (!user || user.authenticators.length === 0) {
      return res
        .status(404)
        .json({ error: "No biometric credentials found for this account" });
    }
  ...
  try {
    ...
    if (verification.verified) {
      ...
      const token = jwt.sign(
        { id: user.id, email: user.email, is_secure: true },
...
app.post("/api/auth/register", async (req, res) => {
  const { email, password, name, region, role } = req.body;
  try {
    ...
    res.status(400).json({
      error: "Registration failed. Authority already exists or invalid data.",
    });
  }
});
  ...
  try {
    ...
    const vault = await prisma.vault.create({
      data: {
        name,
        description,
        tier: user.role, // Vault inherits owner's authority tier
    ...
    fs.appendFileSync(
      "error-trace.log",
      `[${new Date().toISOString()}] Creation Error: ${error.stack || error}\n`,
    );
...
app.post("/api/vaults/:id/members", authenticateToken, async (req, res) => {
  ...
  try {
    // Check if requester is OWNER/EXECUTIVE and their tier
    const user = await prisma.user.findUnique({ where: { id: req.user.id } });
    ...
    if (!requester || !["OWNER", "EXECUTIVE"].includes(requester.role)) {
      return res
        .status(403)
        .json({ error: "Insufficient clearance to invite members." });
    }

    // Role-based seat enforcement based on Vault Tier
    const vault = await prisma.vault.findUnique({ where: { id: id } });
    const currentMemberCount = await prisma.vaultMembership.count({
      where: { vault_id: id },
    });

    if (vault.tier === "PERSONAL" && currentMemberCount >= 1) {
      return res.status(403).json({
        error: "Personal Tier limit reached: 1 Seat Max. Upgrade to expand command.",
      });
    }
    if (vault.tier === "SMALL_BUSINESS" && currentMemberCount >= 3) {
      return res.status(403).json({
        error: "Small Business Tier limit reached: 3 Seats Max. Institutional upgrade required.",
      });
    }
    ...
    if (!userToInvite)
      return res
        .status(404)
        .json({ error: "Identity not found in global registry." });
    ...
    res
      .status(400)
      .json({
        error: "Invitation protocol failure. Member might already exist.",
      });
  }
// Join individual vault via Sovereign Invite Code
app.post("/api/vaults/join", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!vault)
      return res.status(404).json({ error: "Sovereign Invite code invalid." });

    // Enforce tier limits
    const currentMemberCount = vault.members.length;
    if (vault.tier === "PERSONAL" && currentMemberCount >= 1) {
      return res.status(403).json({ error: "Vault core capacity reached." });
    }
    if (vault.tier === "SMALL_BUSINESS" && currentMemberCount >= 3) {
      return res.status(403).json({ error: "Vault executive seats occupied." });
    }
    ...
    res.json({ success: true, vault_id: vault.id, name: vault.name });
  } catch (error) {
    res.status(400).json({ error: "Invite redemption failed. Already a member?" });
  }
});
...
app.get("/api/receipts", authenticateToken, async (req, res) => {
  ...
  try {
    if (vault_id) {
      ...
      if (!membership)
        return res.status(403).json({ error: "Vault access denied." });
...
app.post("/api/receipts/manual", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ MANUAL ENTRY: ${merchant} — ${protocol.currency} ${amount}`,
    );
    res.json(receipt);
  } catch (error) {
    ...
    if (receipt.user_id !== req.user.id) {
      if (receipt.vault_id) {
        ...
        if (!membership || membership.role === "AUDITOR") {
          return res
            .status(403)
            .json({ error: "Insufficient clearance to de-authorize." });
        }
      } else {
...
app.post("/api/receipts/scan", authenticateToken, async (req, res) => {
  ...
  console.log(
    `🏛️  Initiating ReceiptTrac Sovereign Audit for ${region} Protocol...`,
  );
  ...
  try {
    if (vault_id) {
      ...
      if (!membership || membership.role === "AUDITOR") {
        return res
          .status(403)
          .json({ error: "Vault clearance level insufficient for archival." });
      }
    }
    ...
    try {
      ...
      const items =
        extractedData.items?.map((i) => i.name.toLowerCase()).join(" ") || "";

      if (
        !extractedData.category ||
        extractedData.category === "General" ||
        extractedData.category === "Misc"
      ) {
        if (
          store.includes("apple") ||
          store.includes("best buy") ||
          items.includes("phone") ||
          items.includes("computer") ||
          items.includes("macbook") ||
          items.includes("ipad") ||
          items.includes("hardware")
        ) {
          extractedData.category = "Technology";
        } else if (
          store.includes("bell") ||
          store.includes("rogers") ||
          store.includes("telus") ||
          items.includes("mobile") ||
          items.includes("internet") ||
          items.includes("data plan") ||
          items.includes("roaming")
        ) {
          extractedData.category = "Telecommunications";
        } else if (
          store.includes("amazon") ||
          store.includes("ups") ||
          store.includes("fedex") ||
          items.includes("shipping") ||
          items.includes("delivery") ||
          items.includes("logistics")
        ) {
          extractedData.category = "Logistics";
        } else if (
          store.includes("shell") ||
          store.includes("petro") ||
          store.includes("chevron") ||
          store.includes("uber") ||
          store.includes("lyft") ||
          store.includes("hilton") ||
          store.includes("marriott") ||
          items.includes("fuel") ||
          items.includes("hotel") ||
          items.includes("travel")
        ) {
          ...
          store.includes("staples") ||
          items.includes("supplies") ||
          items.includes("office") ||
          items.includes("maintenance")
        ) {
      ...
      console.log(
        `✅ SOVEREIGN ROUTING: ${extractedData.store_name} -> [${extractedData.category}]`,
      );
    } catch (aiError) {
      console.warn(
        "⚠️ Local AI failed or timed out. Falling back to protocol simulation.",
        aiError.message,
      );
      // Fallback: Professional simulation if local AI is not warm or model missing
      const total = 125.0 + Math.random() * 500;
    ...
    res
      .status(500)
      .json({ error: "Sovereign processing encountered a vault failure." });
  }
});
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const latestTotal = receipts.reduce(
    ...
    Return ONLY a JSON object with:
    {
      ...
      res.status(500).json({
        error: "AI reasoning engine timed out or encountered an audit breach.",
      });
    }
  },
);
...
app.post("/api/agent/command", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.json({
      reply:
        "Strategic command failed to execute due to an internal audit breach.",
      action: "NONE",
      logs: [
        { type: "error", msg: "AGENT_FAILURE: Connection to LLM_CORE lost." },
      ],
    });
  }
});
...
app.post("/api/receipts/barcode", authenticateToken, async (req, res) => {
  ...
  const BARCODE_REGISTRY = {
    ...
    "0002": { store: "Apple Store", cat: "Equipment", amount: 1299.0 },
    "0003": { store: "Amazon Hub", cat: "Supplies", amount: 24.5 },
    "0004": { store: "Chevron Executive", cat: "Travel", amount: 85.2 },
    "0005": { store: "Hilton Vault", cat: "Travel", amount: 450.0 },
    "0006": { store: "Uber Black", cat: "Travel", amount: 65.0 },
  };
...
import gatekeeperBridge from "./utils/gatekeeperBridge.js";
...
app.get("/api/gatekeeper/logs", authenticateToken, async (req, res) => {
  try {
    ...
    // Ensure files exist
    if (!fs_extra.existsSync(audit_path))
      fs_extra.writeFileSync(audit_path, "# Sovereign Audit Plan\n");
    if (!fs_extra.existsSync(findings_path))
      fs_extra.writeFileSync(findings_path, "# Security Findings\n");
    ...
    res
      .status(500)
      .json({ error: "Gatekeeper memory is currently inaccessible." });
  }
});

app.post("/api/gatekeeper/audit", authenticateToken, async (req, res) => {
  try {
    // 1. Fetch latest receipts for context
    ...
    // 2. Spawn the OpenManus reasoning agent via the bridge
    // This is the 'YOLO' bridge: Node -> Python (Agent) -> Markdown Logs
    gatekeeperBridge
      .runSovereignAudit(latestReceipts)
      .then(() => {
        console.log(`[GATEKEEPER] Audit completed for user ${req.user.id}`);
      })
      .catch((err) => {
        console.error(`[GATEKEEPER] Audit crashed:`, err);
      });

    res.json({
      success: true,
      msg: "Sovereign Audit Initialized. Check Findings Log for Thinking Trace.",
    });
  } catch (err) {
    res.status(500).json({ error: "Sovereign Engine failed to initialize." });
  }
});
