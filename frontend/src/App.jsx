  ...
  if (loading) {
    return (
      ...
      >
        ...
        <h3 className="text-red-500 font-bold tracking-[0.2em] text-xs uppercase">
          {t("SYSTEM ANALYSIS IN PROGRESS")}...
        </h3>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-lg font-bold text-white tracking-tight uppercase leading-none mb-1">
            {t("DEEP_INSIGHT")}
          </h2>
          <div className="flex items-center gap-2">
            <span
              className={`w-1.5 h-1.5 rounded-full ${insight.fiscal_health === "OPTIMAL" ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]" : insight.fiscal_health === "WATCH" ? "bg-yellow-500" : "bg-red-500"}`}
            />
            <p className="text-[10px] text-gold/60 font-black tracking-widest uppercase">
              {t("FISCAL_HEALTH")}: {insight.fiscal_health}
            </p>
          </div>
        </div>
      </div>

      <p className="text-[11px] text-gray-400 mb-6 italic border-l-2 border-red-500/30 pl-4 py-1 leading-relaxed">
        {insight.summary}
      </p>
            ...
            <div className="flex justify-between items-center pt-3 border-t border-white/5">
              ...
              <span className="text-xs font-black text-green-400 font-mono tracking-tighter">
                {new Intl.NumberFormat(undefined, {
                  style: "currency",
                  currency: "CAD",
                }).format(insight.projected_savings)}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="pt-6 border-t border-gold/10">
        ...
        <ul className="space-y-2">
          {insight.spending_trends.map((trend, idx) => (
            <li
              key={idx}
              className="flex items-start gap-2 text-[10px] text-white/50 leading-tight"
            >
...
const App = () => {
  ...
  const [gatekeeperLogs, setGatekeeperLogs] = useState({
    audit: "",
    findings: "",
  });
  ...
  const fetchGatekeeperLogs = async () => {
    try {
      const resp = await fetch("/api/gatekeeper/logs", {
  ...
  const fetchReceipts = async () => {
    try {
      const url = currentVault
        ? `/api/receipts?vault_id=${currentVault.id}`
        : "/api/receipts";
  ...
  const runSovereignAudit = async () => {
    ...
    setCommandLogs((prev) => [
      ...prev,
      { type: "sys", msg: "SOVEREIGN_GATEKEEPER_ENGAGED." },
    ]);
    ...
    try {
      ...
      // Implementation of live polling while audit is running
      const pollInterval = setInterval(fetchGatekeeperLogs, 3000);

      // Auto-stop polling after 30s as a fallback, or we could wait for completion
      setTimeout(() => {
      ...
      if (resp.ok) {
        setCommandLogs((prev) => [
          ...prev,
          { type: "sys", msg: "SOVEREIGN_AUDIT_INITIALIZED_TRACE_ACTIVE." },
        ]);
      }
    } catch (e) {
      ...
      setCommandLogs((prev) => [
        ...prev,
        { type: "error", msg: "SOVEREIGN_AUDIT_FAIL_RETRY." },
      ]);
    }
  };
  ...
  const enforceCompliance = (id) => {
    ...
    setCommandLogs((prev) => [
      ...
      {
        type: "sys",
        msg: `EXECUTING COMPLIANCE_ENFORCEMENT ON AUDIT:${id.substring(0, 8)}...`,
      },
  ...
  const fetchDeepInsight = async () => {
    ...
    try {
      const url = currentVault
        ? `/api/analytics/predictive?vault_id=${currentVault.id}`
    ...
    try {
      ...
      alert(
        err.response?.data?.error ||
          "Authentication failed. Please verify your Sovereign Key.",
      );
    }
  };
  ...
  const enrollBiometrics = async () => {
    try {
      ...
      alert(
        "Sovereign Biometrics SECURED. You can now access your vault with FaceID or TouchID.",
      );
    } catch (err) {
      console.error("Biometric Error:", err);
      const errorMsg =
        err.response?.data?.error || err.message || "Unknown protocol error";
      alert(
        `Biometric enrollment failed: ${errorMsg}\n\nEnsure your device supports biometrics and you are using a secure context (localhost).`,
      );
    }
  };
  ...
  const verifySecureSession = async () => {
    ...
    try {
      const { data: options } = await api.post(
        "/api/auth/biometric/login-options",
        { email: user.email },
      );
      const asseResp = await startAuthentication(options);
      const { data: res } = await api.post(
        "/api/auth/biometric/upgrade",
        asseResp,
      );
  ...
  const handleBiometricLogin = async () => {
    if (!loginEmail)
      return alert(
        "Enter your executive email to initiate biometric handshake.",
      );
    try {
      const { data: options } = await axios.post(
        "http://localhost:5001/api/auth/biometric/login-options",
        { email: loginEmail },
        { withCredentials: true },
      );
      const asseResp = await startAuthentication(options);
      const { data: res } = await axios.post(
        "http://localhost:5001/api/auth/biometric/login-verify",
        asseResp,
        { withCredentials: true },
      );
      ...
      alert(
        err.response?.data?.error ||
          "Biometric verification failure. Sovereign access denied.",
      );
    }
  };
  ...
  const handleCreateVault = async (e) => {
    ...
    try {
      ...
      setVaults((prev) => [
        ...prev,
        { ...res.data, userRole: "OWNER", memberCount: 1 },
      ]);
      setNewVaultName("");
  ...
  const handleJoinVault = async (e) => {
    ...
    setCommandLogs((prev) => [
      ...prev,
      { type: "sys", msg: "INITIATING INVITE REDEMPTION PROTOCOL..." },
      { type: "agent", msg: "DECRYPTING SOVEREIGN DELEGATION KEY..." },
    ]);
    try {
      const res = await api.post("/api/vaults/join", {
      ...
      setCommandLogs((prev) => [
        ...
        {
          type: "success",
          msg: `VAULT_LINK_ESTABLISHED: Successfully synchronized with ${res.data.name}.`,
        },
      ]);
    } catch (err) {
      const errorMsg = err.response?.data?.error || "Invite redemption failed.";
      setCommandLogs((prev) => [
        ...prev,
        { type: "error", msg: `PROTOCOL_FAILURE: ${errorMsg}` },
      ]);
      alert(errorMsg);
    }
  };
  ...
  const handleInviteMember = async (e) => {
    ...
    try {
      await api.post(`/api/vaults/${currentVault.id}/members`, {
        email: inviteEmail,
      });
      setInviteEmail("");
  ...
  const handleFileUpload = async (e) => {
    ...
    reader.onloadend = async () => {
      ...
      setCommandLogs((prev) => [
        ...
        {
          type: "agent",
          msg: "LLM_NODE: ANALYZING OPTICAL DATA STRUCTURES...",
        },
      ]);
      ...
      try {
        ...
        setTimeout(() => {
          ...
          setCommandLogs((prev) => [
            ...
            {
              type: "agent",
              msg: `SMART_ROUTING: ${res.data.store_name} identified.`,
            },
            {
              type: "success",
              msg: `ASSET_SECURED: Routed to ${res.data.category.toUpperCase()} Vault.`,
            },
          ]);
        ...
        setCommandLogs((prev) => [
          ...prev,
          { type: "sys", msg: "ERROR: DECRYPTION BREACH." },
        ]);
      }
    };
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setCommandLogs((prev) => [
          ...
          {
            type: "agent",
            msg: `REGISTRY_MATCH: ${res.data.store_name} confirmed.`,
          },
          {
            type: "success",
            msg: `ROUTING COMPLETE: Filed under ${res.data.category.toUpperCase()}.`,
          },
        ]);
      ...
      setCommandLogs((prev) => [
        ...prev,
        { type: "sys", msg: "ERROR: BARCODE UNRECOGNIZED." },
      ]);
    }
  };
  ...
  const handleManualEntry = async (e) => {
    ...
    try {
      ...
      setCommandLogs((prev) => [
        ...
        {
          type: "success",
          msg: `MANUAL_ENTRY: ${manualMerchant} archived at $${manualAmount}.`,
        },
      ]);
      setTimeout(() => setShowFlash(false), 500);
      setTimeout(() => setRecentlyScannedId(null), 5000);
  ...
  const executeStrategicCommand = async (e) => {
    ...
    try {
      ...
      if (action === "DETAIL" && receipts.length > 0)
      ...
      setCommandLogs((prev) => [
        ...
        {
          type: "error",
          msg: "COMMAND_ABORTED: NEURAL_LINK_ESTABLISHMENT_FAILURE",
        },
      ]);
    } finally {
        ...
        <div className="min-h-screen bg-[#050505] relative text-white overflow-x-hidden">
          ...
          <nav className="fixed top-0 left-0 right-0 p-8 flex justify-between items-center z-50 bg-gradient-to-b from-black/80 to-transparent backdrop-blur-md">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-gold/10 border border-gold/40 shadow-[0_0_15px_rgba(255,40,0,0.2)] rounded-full flex items-center justify-center font-bold text-gold text-[10px] tracking-tighter">
                RT
              </div>
              <span className="gold-text tracking-[0.3em] text-xs italic uppercase">
                ReceiptTrac
              </span>
            </div>
            <button
          ...
          <section className="relative h-screen flex flex-col items-center justify-center text-center p-6">
            ...
            >
              ...
              <h1
                className="text-8xl md:text-[10rem] font-bold gold-foil tracking-tighter mb-4"
              ...
              <div className="flex items-center justify-center gap-4 mb-12">
                ...
                <p className="gold-label tracking-[0.5em] text-xs">
                  Sovereign Financial Vision
                </p>
                <div className="h-[1px] w-24 bg-gold/20" />
              </div>

              <p className="max-w-2xl mx-auto text-lg text-white/40 font-light leading-relaxed mb-12 italic">
                The absolute standard in jurisdictional fiscal oversight.
                Powered by local AI to ensure your data stays within the
                executive borders.
              </p>
          ...
          <section className="py-40 bg-[#020202] border-y border-gold/5 relative overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-20 text-center relative z-10">
              <div className="group">
                <h3 className="gold-text text-6xl mb-4 transition-transform group-hover:scale-110 duration-700">
                  100%
                </h3>
                <p className="gold-label opacity-40 mb-2">
                  Privacy Sovereignty
                </p>
                <div className="w-8 h-[1px] bg-gold/20 mx-auto mb-4" />
                <p className="text-[10px] opacity-30 leading-relaxed uppercase tracking-widest">
                  Local AI Protocol • On-Premise Compute
                </p>
              </div>
              <div className="md:border-x border-white/5 group">
                ...
                <div className="w-8 h-[1px] bg-gold/20 mx-auto mb-4" />
                <p className="text-[10px] opacity-30 leading-relaxed uppercase tracking-widest">
                  Isolated Vault • Zero External Latency
                </p>
              </div>
              <div className="group">
                <h3 className="gold-text text-6xl mb-4 transition-transform group-hover:scale-110 duration-700">
                ...
                <div className="w-8 h-[1px] bg-gold/20 mx-auto mb-4" />
                <p className="text-[10px] opacity-30 leading-relaxed uppercase tracking-widest">
                  GST • QST • HST • US GLOBAL RECOGNITION
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="py-32 flex flex-col items-center justify-center text-center p-12">
            ...
            >
              ...
              <p className="text-white/60 leading-loose text-lg mb-8 italic">
                "In an age of surveillance, ReceiptTrac stands as a sentinel of
                financial privacy. We believe in tools that work for you, not
                those that report you. Your receipts are your records. Your data
                is your territory. We only provide the lens."
              </p>
          ...
          <section className="py-40 relative">
            <div className="max-w-6xl mx-auto px-6">
              <div className="text-center mb-24">
                <h2
                  className="text-5xl font-bold gold-foil mb-4"
                  style={{ fontFamily: "'Cormorant Garamond', serif" }}
                >
                  Executive Investment
                </h2>
                <div className="flex items-center justify-center gap-4">
                  ...
                  <p className="gold-label tracking-[0.4em] text-[10px] uppercase">
                    Secure Subscription Tiers
                  </p>
                  <div className="h-[1px] w-12 bg-gold/20" />
                </div>
              </div>
                ...
                {[
                  {
                    ...
                    price: "0",
                    unit: "/ sovereign",
                    seats: "1 Sovereign Seat",
                    features: [
                      "Local AI Intelligence",
                      "Single Vault Archive",
                  ...
                  },
                  {
                    ...
                    price: "49",
                    unit: "/ period",
                    seats: "3 Executive Seats",
                    features: [
                      "Deep Insight AI Engine",
                      "Quarterly Forecasting",
                      "Executive PDF/XLSX",
                      "Priority Local Scan",
                    ],
                    cta: "Secure Authority",
                    premium: true,
                  },
                  {
                    name: "Institutional",
                    price: "199",
                    unit: "/ month",
                    seats: "Unlimited Sentinels",
                    features: [
                      "Swarm Auditing",
                      "Multi-Region Arbitrage",
                      "24/7 Gatekeeper Sync",
                      "Institutional Board",
                    ],
                    cta: "Expand Empire",
                    premium: false,
                  ...
                  <motion.div
                    key={i}
                    whileHover={{ y: -10 }}
                    className={`leather-card p-12 flex flex-col items-center text-center ${tier.premium ? "border-red-500/40 ring-1 ring-red-500/20 shadow-[0_0_50px_rgba(255,40,0,0.1)]" : ""}`}
                  >
                    <h3 className="gold-heading text-2xl mb-2">{tier.name}</h3>
                    <div className="flex items-start justify-center gap-1 mb-4">
                      <span className="text-xl gold-label mt-2 leading-none">
                        $
                      </span>
                      <span className="text-6xl font-black gold-text leading-none">
                        {tier.price}
                      </span>
                      <span className="text-xs gold-label opacity-40 self-end mb-2">
                        {tier.unit}
                      </span>
                    </div>

                    <div className="mb-8 px-4 py-1 bg-gold/10 border border-gold/20 rounded-full">
                      <span className="text-[9px] gold-label tracking-[0.2em] uppercase font-bold">
                        {tier.seats}
                      </span>
                    </div>
                    ...
                    <ul className="space-y-4 mb-12 flex-1">
                      {tier.features.map((f, j) => (
                        <li
                          key={j}
                          className="text-[10px] gold-label opacity-40 uppercase tracking-widest flex items-center gap-2"
                        >
                    ...
                    <button
                      onClick={() => setShowLogin(true)}
                      className={`w-full py-4 tracking-[0.3em] text-[10px] font-bold uppercase transition-all ${tier.premium ? "bg-red-500 text-white shadow-[0_0_15px_rgba(255,40,0,0.4)] hover:bg-red-600" : "border border-gold/20 text-gold hover:bg-gold/5"}`}
                    >
                      {tier.cta}
                    </button>
                  </motion.div>
            ...
            <div className="max-w-5xl mx-auto px-8 flex flex-wrap justify-center gap-16 opacity-40 grayscale hover:grayscale-0 transition-all duration-1000 relative z-10">
              {[
                ...
                >
                  ...
                  <span className="text-[9px] tracking-[0.4em] gold-label font-bold whitespace-nowrap">
                    {badge.label}
                  </span>
                </div>
              ))}
          ...
          <footer className="p-16 text-center border-t border-white/5 relative overflow-hidden">
            ...
            <p className="text-[10px] gold-label opacity-20 tracking-[0.5em] mb-4">
              ReceiptTrac ARCHIVES • SOVEREIGN JURISDICTION
            </p>
            <p className="text-[8px] italic opacity-10">
              Established 2026 • Designed for Continuous Oversight
            </p>

            {/* Subtle Signature Accent */}
            <div className="mt-12 opacity-[0.03] pointer-events-none select-none">
              <span
                className="text-6xl font-serif text-red-500"
                style={{ fontFamily: "'Cormorant Garamond', serif" }}
              >
                Lacoste Executive
              </span>
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#050505] relative overflow-hidden">
        ...
        >
          <div className="text-center mb-12 relative z-10">
            ...
            <h1
              className="text-5xl font-bold gold-foil mb-2 tracking-tighter"
              style={{ fontFamily: "'Cormorant Garamond', serif" }}
            >
              ReceiptTrac
            </h1>
          ...
          <div className="leather-card relative overflow-hidden">
            <form onSubmit={handleAuth} className="space-y-6 relative z-10">
              {authMode === "register" && (
                <div>
                  <label className="gold-label mb-2 block tracking-widest">
                    Full Name
                  </label>
                  <input
                    type="text"
              ...
              <div>
                <div className="flex justify-between items-end mb-2">
                  <label className="gold-label tracking-widest block">
                    Sovereign Key
                  </label>
                  {authMode === "login" && (
                    <button
                      ...
                      onClick={() =>
                        alert(
                          "Sovereign Recovery Mode initiated. Please contact your Director or use biometric override.",
                        )
                      }
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  className="btn-gold flex-1 mt-4 flex items-center justify-center gap-2"
                >
                  <Zap size={16} />
                  {authMode === "login"
                    ? "DECRYPT VAULT"
            ...
            <div className="mt-8 text-center">
              ...
              >
                {authMode === "login"
                  ? "Need to provision new authority?"
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#050505]">
        ...
        >
          ...
          <div className="md:w-1/2 p-12 bg-black/40 border-r border-white/5 flex flex-col justify-center">
            <div className="mb-12">
              <h1
                className="text-7xl font-bold gold-foil mb-2 tracking-tighter"
                style={{ fontFamily: "'Cormorant Garamond', serif" }}
              >
                ReceiptTrac
              </h1>
              <div className="stitch-line" />
              <p className="gold-label opacity-60 mt-3">
            ...
            <div className="space-y-4">
              ...
              >
                <div className="flex justify-between items-start">
                  <div
                    className="font-bold text-2xl gold-heading"
                    style={{ fontFamily: "'Cormorant Garamond', serif" }}
                  >
                    Enterprise Vault
                  </div>
                  <span className="text-[10px] gold-label opacity-60">
                    $199 / MO
                  </span>
                </div>
                <div className="gold-label mt-1">
                  Full Auditing • Multi-Region • Organization
                </div>
              </button>
              ...
              >
                ...
                <div className="gold-label mt-1">
                  Tax Recovery • Executive Insights • Cashflow
                </div>
              </button>
              ...
              >
                <div className="flex justify-between items-start">
                  <div
                    className="font-bold text-2xl gold-heading opacity-50 hover:opacity-100 transition-opacity"
                    style={{ fontFamily: "'Cormorant Garamond', serif" }}
                  >
                    Personal Wealth
                  </div>
                  <span className="text-[10px] gold-label opacity-40">
                    $0 / SOV
                  </span>
                </div>
                <div className="gold-label mt-1 opacity-40">
                  Simple Tracking • Daily Budgets • Local AI
                </div>
              </button>
            </div>
          </div>
            ...
            <AnimatePresence mode="wait">
              {!role ? (
                ...
                >
                  ...
                  Please define your access level to see available
                  jurisdictions.
                </motion.div>
              ) : !country ? (
                <motion.div
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-6 max-w-md mx-auto">
                    ...
                    >
                      <svg
                        viewBox="0 0 24 24"
                        className="w-14 h-14 fill-rose-600 grayscale group-hover:grayscale-0 transition-all drop-shadow-[0_0_10px_rgba(225,29,72,0.2)] group-hover:drop-shadow-[0_0_15px_rgba(225,29,72,0.4)]"
                      >
                      ...
                      <span className="gold-heading text-sm opacity-60 group-hover:gold-foil transition-all tracking-[0.1em]">
                        The North
                      </span>
                    </button>
                    <button
                    ...
                    >
                      <div className="relative">
                        <svg
                          viewBox="0 0 20 12"
                          className="w-14 h-auto rounded-sm grayscale group-hover:grayscale-0 transition-all border border-white/5 shadow-lg shadow-black/40"
                        >
                ...
                >
                  ...
                  <div
                    className={`grid gap-4 ${country === "CANADA" ? "grid-cols-2" : "grid-cols-1"}`}
                  >
                    {[
                      {
                        id: "QUEBEC",
                      ...
                      },
                      {
                        ...
                        icon: (
                          <svg
                            viewBox="0 0 24 24"
                            className="w-10 h-10 fill-rose-600"
                          >
                            <path d="M12,2L12.85,5.8L14.5,5.35L13.75,7.7L18.5,8.1L15.1,9.65L16.25,12L12,10.5L7.75,12L8.9,9.65L5.5,8.1L10.25,7.7L9.5,5.35L11.15,5.8L12,2M12,19V22H11V19H12Z" />
                            <path d="M12,10.5L12.5,13L15.5,12.5L13.5,14L14.5,17L12,15L9.5,17L10.5,14L8.5,12.5L11.5,13L12,10.5Z" />
                          </svg>
                      ...
                      },
                      {
                        ...
                        icon: (
                          <svg
                            viewBox="0 0 20 12"
                            className="w-10 h-auto rounded-sm shadow-sm group-hover:shadow-gold/20 transition-all border border-white/5"
                          >
                            <rect width="20" height="12" fill="#fff" />
                            <rect width="20" height="1" y="0" fill="#B22234" />
                            <rect width="20" height="1" y="2" fill="#B22234" />
                      ...
                      .map((loc) => (
                        ...
                        >
                          ...
                          <ChevronRight
                            className="opacity-10 group-hover:opacity-100 group-hover:text-gold transition-all"
                            size={20}
                          />
                        </button>
                      ))}
                  ...
                  <button
                    ...
                    className="gold-label opacity-20 hover:opacity-100 transition-opacity w-full text-center mt-4"
                  >
                    Change Territory
                  </button>
                </motion.div>
  ...
  return (
    <div className="min-h-screen w-full bg-transparent relative overflow-hidden flex flex-col crt-flicker">
      ...
      >
        ...
        <AnimatePresence>
          {isScanning && (
            ...
            >
              <div className="text-center">
                <Zap
                  className="text-red-500 mx-auto mb-4 animate-pulse drop-shadow-[0_0_15px_rgba(255,40,0,0.8)]"
                  size={48}
                />
                <h2
                  className="text-2xl font-black gold-text tracking-[.5em] uppercase"
                  style={{ fontFamily: "'Playfair Display', serif" }}
                >
                  Local AI Auditing...
                </h2>
                <p className="gold-label mt-2 opacity-50 tracking-widest">
        ...
        <header className="p-8 border-b border-gold/10 flex justify-between items-center bg-black/40 backdrop-blur-xl sticky top-0 z-50">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1
                className="text-4xl font-black tracking-[0.3em] gold-text italic"
                style={{ fontFamily: "'Playfair Display', serif" }}
              >
                ReceiptTrac
              </h1>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="gold-label text-[9px] tracking-[0.4em] opacity-40 uppercase">
                    {t("EXECUTIVE SYSTEM")}: {t("SECURED")}
                  </p>
                </div>
                <button
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <Globe
                size={16}
                className={`text-red-500 transition-all ${i18n.language === "fr" ? "rotate-180" : ""}`}
              />
              <span className="gold-label text-[10px] tracking-[0.2em] font-bold">
                {i18n.language.toUpperCase()}
              </span>
            </button>

            <div className="flex items-center gap-3 p-3 sovereign-terminal border-red-500/20 hover:bg-red-950/5 transition-all group relative min-w-[200px]">
              <div className="flex items-center gap-2">
                <ShieldCheck
                  size={14}
                  className={`${user?.is_secure ? "text-red-500" : "text-white/20"} opacity-60 group-hover:opacity-100 transition-all drop-shadow-[0_0_8px_rgba(255,40,0,0.4)]`}
                />
                <div className="flex flex-col">
                  <span className="amber-alert opacity-40 uppercase tracking-widest leading-tight">
                    {user?.is_secure
                      ? "SECURE_VAULT_ACTIVE"
                      : "IDLE_PROTECTION"}
                  </span>
                  <select
                    ...
                    className="bg-transparent text-white text-[10px] font-black focus:outline-none appearance-none cursor-pointer tracking-wider sovereign-mono"
                  >
                    <option value="personal" className="bg-[#020202] text-white/60">
                      {t("PERSONAL_VAULT")}
                    </option>
                    {vaults.map((v) => (
                      <option key={v.id} value={v.id} className="bg-[#020202] text-red-500 font-bold">
                        {v.name.toUpperCase()}
                      </option>
                    ))}
                    <option
                      value="join"
                      className="bg-[#020202] text-[#ff8c00] italic font-bold"
                    >
                      >> REDEEM_INVITE
                    </option>
                    <option
                      value="new"
                      className="bg-[#020202] text-red-600 italic"
                    >
                      >> {t("CREATE_VAULT")}
                    </option>
                  </select>
            ...
            <div className="flex items-center gap-3 p-3 leather-card border-none hover:bg-white/5 transition-all group relative">
              <div className="flex items-center gap-2">
                <ArrowRightLeft
                  size={14}
                  className="text-red-500 opacity-60 group-hover:opacity-100 transition-all"
                />
                <div className="flex flex-col">
                  ...
                  >
                    <option value="CAD" className="bg-[#0a0a0a]">
                      CAD
                    </option>
              ...
              <div className="flex flex-col border-l border-white/5 pl-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[9px] gold-mono text-red-500/60">
                    {targetCurrency === "CAD"
                      ? `USD: ${(1 / exchangeRates.USD).toFixed(3)}`
                      : `CAD: ${(exchangeRates[targetCurrency] || 1).toFixed(3)}`}
                  </span>
            ...
            <p className="gold-label opacity-60 flex items-center gap-2">
              ...
              {t("AUTHENTICATED")}:{" "}
              <span className="gold-text font-bold uppercase">
                {user?.name}
              </span>
            </p>
          </div>
          ...
          <div className="flex gap-4">
            <button
              ...
              className="p-3 leather-card hover:bg-white/5 transition-colors border-none shadow-none group"
            >
              <LogOut
                size={18}
                className="text-red-500 opacity-40 group-hover:opacity-100 transition-all"
              />
            </button>
            <label
              htmlFor="receipt-upload-header"
              className="gold-hardware flex items-center gap-2 px-8 py-3 rounded-none overflow-hidden relative shadow-[0_0_20px_rgba(212,175,55,0.2)] cursor-pointer"
            >
              <div className="absolute inset-0 bg-white/10 opacity-0 hover:opacity-100 transition-opacity" />
              <input
                type="file"
                accept="image/*"
                id="receipt-upload-header"
                className="hidden"
              ...
              />{" "}
              <span className="text-[10px] tracking-[0.2em] font-bold text-black">
                {t("DEPOSIT VOUCHER")}
              </span>
            </label>
          </div>
        ...
        >
          <div className="leather-card p-6 border-b-2 border-b-red-500/30">
            <p className="gold-label text-[10px] opacity-40 mb-1">
              {t("FISCAL VOLUME")}
            </p>
            <div className="flex items-end gap-2 text-red-500">
              ...
              <span className="gold-label text-[10px] mb-1">
                {t("VOUCHERS")}
              </span>
            </div>
          </div>
                  ...
                  receipts.reduce(
                    (a, r) =>
                      ...
                      convertAmount(
                        ...
                        r.currency || (r.region === "USA" ? "USD" : "CAD"),
                      ),
                    0,
                  ),
                )}
        ...
        <div className="flex flex-col md:flex-row gap-6 mb-12 items-center justify-between">
          <div className="relative w-full md:w-1/2">
            <Search
              className="absolute left-6 top-1/2 -translate-y-1/2 text-red-500/40"
              size={18}
            />
            <input
              type="text"
          ...
          <div className="flex gap-4 w-full md:w-auto">
            ...
            >
              ...
              <span className="text-[10px] font-bold tracking-widest uppercase">
                {viewMode === "table" ? t("CARD VIEW") : t("TABLE VIEW")}
              </span>
            </button>
            <button
              onClick={exportExcel}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 space-y-12">
              ...
              <div className="space-y-8">
                ...
                <div className="leather-card bg-black/80 border-red-500/20 h-[500px] flex flex-col overflow-hidden relative shadow-[0_0_50px_rgba(255,40,0,0.1)]">
                  <div className="p-3 border-b border-red-500/10 flex items-center justify-between bg-red-500/5">
                    ...
                    <span className="text-[9px] font-bold text-red-500/50 tracking-[0.4em] font-mono pulse">
                      STRATEGIC_COMMAND_CONSOLE_V2
                    </span>
                  </div>
                ...
                `}
                    ...
                    {isAgentWorking && (
                      <div className="text-red-500 animate-pulse">
                        $ THINKING... [AGENTIC_PROCESS_ACTIVE]
                      </div>
                    )}
                  ...
                  <form
                    onSubmit={executeStrategicCommand}
                    className="p-3 bg-red-500/5 border-t border-red-500/10 flex items-center gap-3"
                  >
            ...
            <div
              className={`leather-card ${role === "PERSONAL" ? "hidden" : ""}`}
            >
            ...
            >
              ...
              <div className="mt-4 grid grid-cols-2 gap-4">
                <div className="p-3 bg-white/5 border border-white/5">
                  <div className="gold-label opacity-40">
                    {t("TAX INTENSITY")}
                  </div>
            ...
            <div
              className={`leather-card ${role === "PERSONAL" ? "border-gold/40 shadow-[0_0_30px_rgba(212,175,55,0.1)]" : ""}`}
            >
              <h2 className="text-lg font-bold mb-6 flex items-center gap-3 uppercase tracking-widest text-gold/80">
                <Camera size={18} />{" "}
                {role === "PERSONAL" ? "Urgent Submission" : t("DOCUMENT SCAN")}
              </h2>
              <div className="file-input-wrapper">
                ...
                <label
                  ...
                  className="file-input group cursor-pointer h-32 flex flex-col justify-center items-center rounded-lg border-2 border-dashed border-red-500/30 hover:border-red-500 bg-black/30 hover:bg-red-500/5 transition-all"
                >
                  <div className="text-center">
                    <Plus
                      size={40}
                      className="mx-auto mb-4 text-red-500 opacity-40 group-hover:opacity-100 transition-all duration-500 group-hover:scale-125"
                    />
                    <p className="gold-label opacity-40 group-hover:opacity-100 transition-opacity tracking-widest text-[9px]">
                      {role === "PERSONAL"
                        ? "DROP RECEIPT FOR APPROVAL"
                        : t("DEPOSIT DIGITAL VOUCHER")}
                    </p>
                  </div>
                </label>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {activeCategory && (
                    <button
                      ...
                      className="text-[9px] text-red-500/60 hover:text-red-500 transition-colors ml-1 uppercase tracking-widest"
                    >
                      ✕ Clear
                    </button>
                  )}
                ...
                <div className="space-y-8">
                  <div className="flex justify-between items-end mb-4">
                    <div className="space-y-1">
                      ...
                      <p
                        className="text-4xl font-bold gold-foil tracking-tighter"
                        style={{ fontFamily: "'Cormorant Garamond', serif" }}
                      >
                        {new Intl.NumberFormat(undefined, {
                          style: "currency",
                  ...
                  <div className="grid grid-cols-1 gap-8 py-4">
                    ...
                    <div className="h-48 relative">
                      <p className="gold-label absolute top-0 left-0 text-[9px] opacity-40">
                            ...
                            data={[
                              {
                                ...
                                value: receipts.filter(
                                  (r) =>
                                    r.category === "Business" ||
                                    r.category === "Professional Services",
                                ).length,
                              },
                              {
                                name: "Travel",
                      ...
                      <div className="absolute top-[60%] left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
                        <BarChart3
                          className="text-red-500 opacity-20 mx-auto drop-shadow-[0_0_10px_rgba(255,40,0,0.5)]"
                          size={24}
                        />
                      </div>
                    </div>
                  ...
                  <div className="pt-6 border-t border-white/5 space-y-4">
                    <div className="flex justify-between text-[10px] uppercase tracking-widest">
                      <span className="gold-label opacity-40">
                        Tax Recoverability
                      </span>
                    ...
                    <p className="text-[8px] text-text-dim mt-2 opacity-50">
                      AI has verified all jurisdictional compliance for the{" "}
                      {region} protocol.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4">
                    <div className="p-3 bg-white/5 border border-white/5">
                      ...
                      <div className="text-xl font-bold gold-mono">
                        {receipts.length}
                      </div>
                    </div>
                    <div className="p-3 bg-white/5 border border-white/5">
              ...
              currentVault && (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-8">
                    <div>
                      <h3 className="gold-heading uppercase tracking-[0.2em] text-[10px] opacity-60 mb-1">
                        {t("EXECUTIVE_BOARD")}
                      </h3>
                      <h2
                        className="text-xl font-bold gold-text tracking-tighter"
                        style={{ fontFamily: "'Cormorant Garamond', serif" }}
                      >
                        {currentVault.name}
                      </h2>
                    ...
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(currentVault.invite_code);
                        // We could add a toast here, but for now we'll stick to the logic
                      }}
                      className="sovereign-terminal px-4 py-3 hover:bg-red-950/20 transition-all flex items-center gap-6 group/invite border border-red-500/20 min-w-[180px]"
                      title="Copy Delegation Key"
                    >
                      <div className="flex flex-col items-start gap-1">
                        <span className="amber-alert opacity-60 group-hover/invite:opacity-100 transition-opacity">
                          KEY_PROVISIONED
                        </span>
                        <span className="sovereign-mono text-[14px] tracking-[0.4em]">
                          {currentVault.invite_code}
                        </span>
                      </div>
                      <div className="p-2 border border-red-500/20 rounded bg-red-500/5 group-hover/invite:border-red-500 transition-all">
                        <Copy
                          size={14}
                          className="text-red-500 opacity-40 group-hover/invite:opacity-100 group-hover/invite:animate-pulse"
                        />
                      </div>
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    {currentVault.members?.map((m) => (
                      <div
                        key={m.user_id}
                        className="flex justify-between items-center p-4 bg-gradient-to-r from-white/[0.03] to-transparent border-l-2 border-red-500/40 hover:border-red-500 transition-all group"
                      >
                        <div className="flex items-center gap-4">
                          <div className="relative">
                          ...
                          <div>
                            <p className="text-[12px] font-bold text-white group-hover:text-red-500 transition-colors uppercase tracking-tight">
                              {m.user?.name || m.user?.email}
                            </p>
                            <p className="text-[8px] gold-label opacity-40 uppercase tracking-[0.2em]">
                              {m.role}
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="mt-10 pt-6 border-t border-white/5">
                    ...
                    >
                      ...
                      <Plus
                        size={16}
                        className="group-hover:rotate-90 transition-transform relative z-10"
                      />
                      <span className="relative z-10 font-black tracking-[0.2em]">
                        {t("INVITE_EXECUTIVE")}
                      </span>
                    </button>
                    <p className="text-[8px] gold-label opacity-20 text-center mt-4 tracking-widest uppercase italic">
                      Secure Multi-Executive Access Active
                    </p>
                  </div>
                </motion.div>
              )}
            ...
            {(role === "ENTERPRISE" || role === "SMALL_BUSINESS") &&
              currentVault && (
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className={`mt-8 space-y-8 ${isAuditing ? "ring-2 ring-red-500/20 shadow-[0_0_50px_rgba(255,40,0,0.1)] rounded-lg p-2 transition-all duration-1000" : ""}`}
                >
                  <div className="grid grid-cols-1 gap-8">
                    <div
                      className={`leather-card bg-black/40 border-gold/10 p-6 min-h-[250px] flex flex-col ${isAuditing ? "animate-pulse" : ""}`}
                    >
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-[10px] font-black tracking-[0.4em] uppercase text-gold/60">
                          {t("GATEKEEPER_AUDIT_PLAN")}
                        </h4>
                        <ShieldAlert
                          size={14}
                          className={
                            isAuditing
                              ? "text-gold animate-bounce"
                              : "text-gold/40"
                          }
                        />
                      </div>
                    ...
                    <div
                      className={`leather-card bg-black/40 border-red-500/10 p-6 min-h-[300px] flex flex-col relative overflow-hidden`}
                    >
                      <div
                        className={`absolute top-0 right-0 w-32 h-32 ${isAuditing ? "bg-red-600/10 animate-pulse" : "bg-red-500/5"} blur-3xl pointer-events-none`}
                      />
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-[10px] font-black tracking-[0.4em] uppercase text-red-500/60">
                          {t("SECURITY_FINDINGS")}
                        </h4>
                        <Activity
                          size={14}
                      ...
                      <div className="flex-1 overflow-auto custom-scrollbar pr-2">
                        <pre className="text-[9px] font-mono text-red-500/40 whitespace-pre-wrap leading-relaxed">
                          {isAuditing &&
                            "🧬 GATEKEEPER_REASONING_IN_PROGRESS...\n\n"}
                      ...
                      <button
                        ...
                        disabled={isAuditing}
                        className={`mt-6 gold-hardware-small py-4 text-[9px] font-black tracking-widest hover:scale-[1.02] active:scale-95 transition-all w-full flex items-center justify-center gap-2 ${isAuditing ? "opacity-50 cursor-wait bg-red-950/20" : ""}`}
                      >
                        {isAuditing ? (
                          <>
            ...
            {role === "PERSONAL" && (
              <div className="p-8 leather-card opacity-50">
                <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/40 mb-4">
                  Staff Insight Locked
                </h3>
                <p className="text-xs italic opacity-40">
                  Organizational summaries and tax recovery tools are reserved
                  for Director level access.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="flex justify-between items-center px-2">
              <h2
                className="text-xl font-bold tracking-[0.1em] sovereign-mono"
              >
                {t("AUDIT LEDGER")}
              </h2>
              <div className="flex items-center gap-4">
                <div className="relative group">
                  <Search
                    size={14}
                    className="absolute left-3 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-red-500 transition-colors"
                  />
                  <input
                    type="text"
                ...
                <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    <span className="gold-label text-[9px] group-hover:text-gold transition-colors">
                      {currencyMode === "LOCAL"
                        ? t("ORIGINAL_PROTOCOL")
                        : t("CONVERTED_TO", { target: targetCurrency })}
                    </span>
                  </button>
                  {(role === "ENTERPRISE" || role === "SMALL_BUSINESS") && (
                    <>
                      ...
                      >
                        ...
                        />
                        <span className="gold-label group-hover:text-gold hidden md:inline">
                          .XLSX
                        </span>
                      </button>
                    </>
              ...
              {isScanning && (
                ...
                >
                  ...
                  <div className="flex items-center gap-6 relative z-10">
                    ...
                    <div>
                      <h3 className="font-bold text-red-500 tracking-wide">
                        {t("SYSTEM ANALYSIS IN PROGRESS")}
                      </h3>
                      <p className="text-xs text-text-dim uppercase tracking-widest">
                        {t("PARSING_REGION", { region })}
                      </p>
                    </div>
                  </div>
                </motion.div>
            ...
            <div className="space-y-4">
              {receipts.length === 0 && !isScanning ? (
                <div className="leather-card text-center py-24 opacity-30 border-dashed border-red-500/20">
                  <FileText
                    size={48}
                    className="mx-auto mb-6 text-red-500 opacity-20"
                  />
                  <p className="gold-label opacity-40">
                    {t("LEDGER CURRENTLY CLEAR")}
                  </p>
                </div>
              ) : (
                    ...
                    <table className="w-full text-left border-separate border-spacing-0">
                          ...
                          {[
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                {sortConfig.key === h.key && (
                                  <ChevronRight
                                    className={`transition-transform duration-300 ${sortConfig.direction === "asc" ? "-rotate-90 text-red-500" : "rotate-90 text-red-500"} drop-shadow-[0_0_5px_rgba(255,40,0,0.5)]`}
                                    size={10}
                                  />
                                )}
                              </div>
                          ...
                          .map((r, i) => (
                            <React.Fragment key={r.id || i}>
                              <tr
                                onClick={() =>
                                  setSelectedReceipt(
                                    selectedReceipt === r.id ? null : r.id,
                                  )
                                }
                              ...
                              >
                                <td className="px-6 py-5 first:pl-10">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div
                                      className={`w-1.5 h-1.5 rounded-full ${r.id ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]" : "bg-red-500 gold-pulse"}`}
                                    />
                                  </div>
                                </td>
                                ...
                                <td className="px-6 py-5">
                                  <div className="flex flex-col">
                                    <span className="text-xs font-bold tracking-wider text-white group-hover:text-red-500 transition-colors uppercase">
                                      {r.store_name?.toUpperCase() ||
                                        t("UNIDENTIFIED ENTITY")}
                                    </span>
                                ...
                                <td className="px-6 py-5">
                                  <div className="flex justify-between items-center group/total">
                                    ...
                                    <ChevronRight
                                      className={`opacity-0 group-hover:opacity-40 transition-all duration-300 ${selectedReceipt === r.id ? "rotate-90 text-red-500 drop-shadow-[0_0_8px_rgba(255,40,0,0.8)] opacity-100" : ""}`}
                                      size={14}
                                    />
                                  </div>
                                </td>
                              ...
                              <AnimatePresence>
                                {selectedReceipt === r.id && (
                                  <tr>
                                    <td
                                      colSpan={6}
                                      className="p-0 border-b border-red-500/20"
                                    >
                                      <motion.div
                                      ...
                                      >
                                        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-12">
                                          ...
                                          <div className="space-y-4">
                                            <div className="aspect-[3/4] bg-white/[0.02] border border-red-500/10 rounded-lg flex items-center justify-center relative group/img overflow-hidden shadow-[inset_0_0_20px_rgba(255,40,0,0.05)]">
                                              ...
                                              <div className="text-center p-8">
                                                <Camera
                                                  size={24}
                                                  className="mx-auto mb-4 text-red-500/30 group-hover/img:text-red-500 transition-colors duration-500 drop-shadow-[0_0_10px_rgba(255,40,0,0.2)]"
                                                />
                                                <p className="text-[10px] text-white/20 tracking-widest font-bold">
                                                  {t("LEGITIMACY VERIFIED")}
                                                </p>
                                              </div>
                                              <div className="verified-watermark">
                                                {t("CERTIFIED AUDIT")}
                                              </div>
                                            </div>
                                          </div>
                                          ...
                                          <div className="space-y-8">
                                              ...
                                              <div className="grid grid-cols-2 gap-y-4">
                                                ...
                                                {Number(r.tax_gst) > 0 && (
                                                  <div className="space-y-1">
                                                    ...
                                                    <p className="gold-mono text-sm text-red-500/80">
                                                      {new Intl.NumberFormat(
                                                        "en-CA",
                                                        {
                                                          style: "currency",
                                            ...
                                            <div className="space-y-4">
                                              <p className="gold-label text-[10px] opacity-40">
                                                {t("EXECUTIVE NOTES")}
                                              </p>
                                            ...
                                            <div className="flex gap-4">
                                              ...
                                              >
                                                <span className="relative z-10 transition-transform duration-300 group-hover:scale-110 block">
                                                  {t("ENFORCE COMPLIANCE")}
                                                </span>
                                              </button>
                                              <button
                                                onClick={(e) =>
                                                  handleDeleteReceipt(r.id, e)
                                                }
                                            ...
                                            {complianceStamps.has(r.id) && (
                                              <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50">
                                                <div className="success-stamp active border-[12px] rotate-[-12deg] opacity-70 p-6 flex flex-col items-center">
                                                  ...
                                                  <span className="text-[10px] tracking-[0.5em]">
                                                    {
                                                      new Date()
                                                        .toISOString()
                                                        .split("T")[0]
                      ...
                      <tfoot className="sticky bottom-0 z-10 bg-[#0a0a0a]">
                        <tr className="border-t-2 border-red-500/20">
                          <td
                            colSpan={4}
                            className="px-6 py-6 gold-label text-red-500 font-black tracking-[0.4em]"
                          >
                            {t("EXECUTIVE PORTFOLIO TOTALS")}
                          </td>
        ...
        <AnimatePresence>
          {isCreateVaultOpen && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
              ...
              >
                <h2 className="gold-heading text-xl mb-6 tracking-widest uppercase">
                  {t("CREATE_VAULT")}
                </h2>
                <form onSubmit={handleCreateVault} className="space-y-6">
                  <div>
                    <label className="gold-label text-[10px] mb-2 block tracking-widest uppercase">
                      {t("VAULT_NAME")}
                    </label>
                    <input
                      autoFocus
          ...
          {isManualEntryOpen && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
              <motion.div
                ...
                className="leather-card w-full max-w-md border-gold/20"
              >
                <h2 className="gold-heading text-xl mb-2 tracking-widest uppercase">
                  Manual Entry
                </h2>
                <p className="gold-label opacity-40 text-[10px] mb-6 tracking-widest">
                  ARCHIVE RECEIPT WITHOUT SCAN
                </p>
          ...
          {isJoinVaultOpen && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
              <motion.div
                ...
                animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0.9, opacity: 0 }}
                className="leather-card sovereign-terminal w-full max-w-md border-red-500/40"
              >
                <div className="flex items-center gap-3 mb-6">
                  <div className="p-2 bg-red-500/10 rounded border border-red-500/20">
                    <Key className="w-5 h-5 text-red-500" />
                  </div>
                  <h2 className="gold-heading text-xl tracking-widest uppercase">
                    Redeem Invite
                  </h2>
                </div>
                <form onSubmit={handleJoinVault} className="space-y-6">
                  <div>
                    <label className="gold-label text-[10px] mb-2 block tracking-widest uppercase opacity-40">
                      Sovereign Invite Code
                    </label>
                    <input
                      autoFocus
                      required
                      ...
                      className="w-full bg-white/5 border border-red-500/10 p-4 font-mono text-red-500 text-lg tracking-[0.3em] focus:border-red-500/40 transition-all focus:outline-none placeholder:opacity-20 mb-4"
                      placeholder="X8J2K9L"
                    />
                    <div className="p-4 bg-red-950/10 border border-red-500/10 rounded-sm">
                      <p className="amber-alert leading-relaxed uppercase">
                        [PROTOCOL_WARNING]: REDEEMING AN INVITE CODE WILL LINK 
                        YOUR ACCOUNT TO THE TARGET VAULT AS AN EXECUTIVE. 
                        ALL ASSETS SCANNED WHILE ACTIVE IN THIS VAULT WILL
                  ...
                  <div className="flex gap-4">
                    ...
                    <button
                      type="submit"
                      className="flex-1 py-4 gold-hardware-small border-red-500/40 text-[10px] font-bold text-red-500 uppercase tracking-widest shadow-[0_0_20px_rgba(255,40,0,0.1)]"
                    >
                      Link Vault
                    </button>
                  </div>
